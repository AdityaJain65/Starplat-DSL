...

{
  ...
  cudaMemcpy(d_BC, BC, sizeof(float)*(V), cudaMemcpyHostToDevice );
  ...
  Compute_BC_kernel<<<numBlocks, threadsPerBlock>>>(V,E,d_sigma);
  ...
  initKernel<float> <<<numBlocks,threadsPerBlock>>>(V,d_sigma,(float));
